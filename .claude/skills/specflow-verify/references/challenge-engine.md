...
Proactive challenges are checklist items with `mode: proactive`. They trigger during `specflow check --proactive`.
...
When `specflow check` reviews an artifact:
...
uv run specflow check --proactive STORY-001
