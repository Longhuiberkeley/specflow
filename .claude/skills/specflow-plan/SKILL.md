...
4. **Assessment lenses** — Apply traceability, completeness, linkage, coverage, and staleness lenses, then a **Risk Profile per change** (run `specflow risk-tier <IDs>` to get the computed tier + reversibility + blast-radius count; then add your confidence + the reason it isn't higher, and persist it on the DEC via `specflow update <DEC> --set risk_profile.confidence=<value>`, which merges into the existing profile). Show ✅/⚠️/❌ results.
...
6. **Risk-proportional gate** — Run `specflow risk-tier <IDs>` to get the computed minimum tier (0 light / 1 normal / 2 stop) and present accordingly; you may escalate above the floor freely, but downgrading below it requires a recorded justification in the DEC's `risk_profile`. For Tier 2 changes, point at the specific concern. Do not calibrate the tier from past approvals.
