...
- **Audit signal design** *(CHL-344)* — make audit findings honest and actionable: same problem class (e.g. REQ with ACs but zero linked tests) must not be `warn` for 8 and `info` for 20 while triaging; per-AC verification rows instead of per-REQ rollups so observability-unclassified ACs are individually actionable; exercise non-functional categories beyond `functional` (18/39 REQs carry none today); and add structural rules behind workflow tags (`backfilled`, `deferred`) so they assert, not just describe. **Trend-delta slice shipped post-v1.13.4**: audit headers report errors/warns/info/chain-coverage deltas vs the prior stamped AUD, and new AUD artifacts stamp machine-readable summary counts (CHL-341 addressed). **A0 lossless-cache slice shipped post-v1.13.5**: the findings cache now stores the full finding set (no 20-finding cap) and is keyed on a bumpable cache generation, so cached replays match fresh-run totals and cannot shadow lens changes; **A1 warn-split stamp slice** followed it: AUD stamps also record escalating vs accounting warns (alongside, not inside, the baseline stamp), so the coming severity unification lands as "escalating 0→0, accounting 0→N" in the trend line instead of a fake warns regression. **A2 severity-unification slice** followed it: ac-coverage unified at WARN across both degrees (one problem class, one severity — the degree now lives in the message text) and stays accounting, so AUD-082's trend reads "escalating 0→0, accounting 0→25" honestly; cache generation bumped with it. First-run observation: title-based CHL dedup is status-blind and matched the addressed CHL-338 (which owns the stable grouped title), so the resurfaced debt minted no fresh tracker — future dedup work should consider limiting suppression to open CHLs. **A3 per-AC-rows slice** followed it: the ac-observability aggregate carries its per-AC items out (single parse path) and the audit report body renders them as an action-bounded appendix — unclassified/aspirational rows only, sorted REQ IDs, full all-class table in subagent-cross-cutting.md — with the findings list, Summary counts, and stamps byte-identical (rows are report body, not findings); suppressed under --quick. **A4 NFR-vocabulary slice** followed it: the `non_functional_category` vocabulary is frozen in code (`NFR_CATEGORIES` in lib/lint.py — source of truth; create-boundary argparse choices generated from the constant; artifact-lint `nfr-category` warn-only typo net for the freeform `--set` path), and nfr-coverage is registered accounting, so its >=50%-missing warn prints and stamps but can never drive exit-2 (cache generation bumped with it). **A5 NFR dogfood sweep** followed it (user-approved, per-artifact semantic review): all 18 REQs omitting `non_functional_category` were walked against the CKL-REV-REQ-02 bar — every one is genuinely a feature/capability requirement, so all 18 were backfilled `functional` and "zero genuine NFRs in this project" is the recorded, honest outcome (NFR histogram now 0/39 missing; mechanical stamping of fancier categories was prohibited). Sweep discovery: artifact fingerprints hash the BODY only, so frontmatter-only edits (this sweep; link/status edits generally) do not invalidate the audit findings cache — cached replays can shadow frontmatter changes until a body changes or the cache is purged; candidate follow-up: fold a frontmatter component into `_project_fingerprint`. **A6 workflow-tag slice** followed it (FINAL code slice — the workstream's code half is closed): the `backfilled` tag now asserts — artifact-lint's `backfilled-links` check warns (never blocks; artifact-lint warns never feed the audit exit gate) on a backfilled artifact with no links in either direction, with a direction carve-out for upstream-less anchor ARCHs (skeleton ARCH is upstream-less by construction yet anchors the adopted chain — the BP/DEC foundational-provenance precedent), and project-audit's `adoption-exemption` lens prints ONE deterministic INFO line counting backfilled STORYs (approved/implemented/verified) that sit outside the REQ-anchored test-link expectations — the previously IMPLICIT exemption (emergent from check_coverage's REQ-anchored predicates; no bypass exists in code) is now explicit, counted, and reviewable every audit, omitted when zero (0 live — both backfilled STORYs are REQ-anchored), INFO deliberately (adopted code permanently lacks tests, so a WARN would cry-wolf for every adoption-pack consumer); cache generation bumped with it. The `deferred` tag half is closed by DOCUMENTATION, not code: the tag does not exist anywhere (verified — go.py's 'deferred' is a wave-lock result key, REQ-DEFERRED-* is a draft-ID slug), and introducing a tag for zero consumers violates the D-18 frozen-vocabulary discipline and the modeless doctrine — there is no rule to assert. Workstream remaining tail (two deferred follow-ups recorded along the way): dedup-status-awareness (A2 first-run observation: title-based CHL dedup is status-blind, so resurfaced debt matching an addressed CHL mints no fresh tracker — consider limiting suppression to open CHLs) and the cache frontmatter component (A5 sweep discovery: frontmatter-only edits do not invalidate the findings cache — candidate fix folds a frontmatter component into `_project_fingerprint`).
