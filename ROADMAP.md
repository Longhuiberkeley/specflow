...
- **Audit signal design — SHIPPED v1.13.6** *(CHL-344 addressed)* — the full workstream (trend deltas, lossless cache + generation key, escalating/accounting stamp split, ac-coverage severity unification at accounting-warn, per-AC observability rows, NFR vocabulary + dogfood sweep, backfilled/`deferred` workflow-tag rules) is summarized in the v1.13.6 section above. Remaining tail — **dedup status-awareness**: title-based CHL dedup is status-blind, so resurfaced debt matching an addressed CHL mints no fresh tracker (consider limiting suppression to open CHLs); **cache frontmatter component**: artifact fingerprints hash the body only, so frontmatter-only edits do not invalidate audit findings-cache replays (candidate fix: fold a frontmatter component into `_project_fingerprint`).
