...
4. **Assessment lenses** — Apply staleness, coverage, and compliance lenses, then a **Risk Profile** for the release (a release is **irreversible** by default — a published tag is a one-way door). Run `specflow risk-tier <IDs>` for the computed tier + blast-radius count, then add your confidence + the reason it isn't higher, and persist it on the release DEC (`specflow update <DEC> --set risk_profile.confidence=<value>` merges into the existing profile). Show ✅/⚠️/❌ results.
