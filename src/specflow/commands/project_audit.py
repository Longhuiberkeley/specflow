"""specflow project-audit — Full-project health review.

STORY-022 minimal stub: absorbs the removed `specflow compliance` command as the
standards-coverage scope. STORY-030 expands this into conversational scope
selection (internal / standards / full), depth tiers, and subagent fan-out with
AUD artifacts.
"""
...
def run(root: Path, args: dict[str, Any]) -> int:
    """Run project-audit in standards-coverage scope."""
    ...
    header = f"Project Audit — Standards Coverage: {standard}"
    if title:
        header += f" — {title}"
