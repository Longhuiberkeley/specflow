---
id: STORY-024
title: Ship /specflow-artifact-review with tiered depth and adversarial lens fan-out
type: story
status: draft
priority: high
tags:
...
New schema: `templates/schemas/challenge.yaml`. Fields include `id`, `title`, `type: challenge`, `status` (open/addressed/accepted/stale), `severity` (info/warn/error), `lens` (which lens produced it), `links` (to target artifact). Lifecycle: `open → addressed → accepted → stale`.

### Composition

`artifact-lint` (deterministic) → `checklist-run` → LLM judgment → optional adversarial lenses. Detect (dead-code, similarity) runs silently inside review as hygiene pre-step; CLI `specflow detect` remains for CI.
...
3. `specflow artifact-review --depth deep --lenses devil,premortem,assumption,redblue <artifact-ids>` fans out 4 subagents in parallel, each producing CHL artifacts tagged with their lens name
...
5. Lens selection UX presents a checklist with estimated token spend before launching subagents
...
- Remaining 12 adversarial lenses from retrospective §9 (future progressive delivery)
